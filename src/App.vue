    ...
    <VApp>
        <VMain>
            <VContainer class="container main-container">
                ...
                <p>
                    <span>你需要一个支持</span>
                    <VImg :src="matrixLogoURL" class="logo logo-matrix"
                ...
                <VContainer class="container card-container">
                    ...
                    <VCard :loading="serverState.state === undefined" class="card" elevation="24"
                           @click="getServerState">
                        <VCardTitle>运行状态</VCardTitle>
                        <VContainer class="container card-icon-container">
                            <VIcon v-if="serverState.state === undefined" color="white"
                                   icon="mdi-sync-circle" size="150"></VIcon>
                            <VIcon v-else-if="serverState.state === 'operational'" color="green" icon="mdi-check-circle"
                                   size="150"></VIcon>
                            <VIcon v-else-if="serverState.state === 'offline'" color="red" icon="mdi-minus-circle"
                                   size="150"></VIcon>
                            <VIcon v-else color="red" icon="mdi-close-circle" size="150"></VIcon>
                        </VContainer>
                        <VContainer class="container card-item-container">
                            <VCardItem class="card-item">
                                <span v-if="serverState.state === undefined">正在获取信息……</span>
                                <span v-else-if="serverState.state === 'operational'">服务器运行正常 :)</span>
                                <span v-else-if="serverState.state === 'offline'">服务器离线，正在维护 :(</span>
                    ...
                    <VCard :loading="serverState.registration === undefined" class="card" elevation="24"
                           @click="getServerState">
                        <VCardTitle>注册入口状态</VCardTitle>
                        <VContainer class="container card-icon-container">
                            <VIcon v-if="serverState.registration === undefined" color="white"
                                   icon="mdi-sync-circle" size="150"></VIcon>
                            <VIcon v-else-if="serverState.registration === 'open'" color="green"
                                   icon="mdi-chevron-up-circle" size="150"></VIcon>
                            <VIcon v-else-if="serverState.registration === 'invitation'" color="yellow"
                                   icon="mdi-alert-circle-check" size="150"></VIcon>
                            <VIcon v-else-if="serverState.registration === 'closed'" color="red" icon="mdi-minus-circle"
                                   size="150"></VIcon>
                            <VIcon v-else color="red" icon="mdi-close-circle" size="150"></VIcon>
                        </VContainer>
                        <VContainer class="container card-item-container">
                            <VCardItem class="card-item">
                                <span v-if="serverState.registration === undefined">正在获取信息……</span>
                                <span v-else-if="serverState.registration === 'open'">注册入口已开放</span>
                                <span
                                    v-else-if="serverState.registration === 'invitation'">注册入口已开放，但需要邀请码</span>
                                <span v-else-if="serverState.registration === 'closed'">服务器暂不开放注册</span>
                                <span v-else>未能获取到信息</span>
                            </VCardItem>
                        </VContainer>
                    </VCard>
        ...
        <VFooter class="container footer-container">
            <div class="column">
                <p>
                    <span>推荐使用</span>
                    <VImg :src="elementLogoURL" class="logo logo-element"
            ...
            <div class="column">
                ...
                <span>本服务器由 <a href="https://github.com/matrix-org/synapse">Synapse</a> 强力驱动</span>
            </div>
        </VFooter>
    </VApp>
    <VContainer class="container alert-container">
        <VFadeTransition>
            <VAlert :model-value="alertState.show" :type="alertState.type" closable density="compact" max-width="50rem">
                {{ alertState.text }}
            </VAlert>
        </VFadeTransition>
    </VContainer>
        ...
        async getServerState() {
            ...
            try {
                ...
                switch (response.status) {
                    case 200:
                        ...
                        this.serverState.latestClientVersionSupported = json["versions"][json["versions"].length - 1];
                        break;
                    case 502:
            ...
            try {
                ...
                switch (response.status) {
                    ...
                    case 401:
                        const json = await response.json();
                        if (json["flows"].some(flow => flow["stages"].includes("m.login.registration_token"))) {
